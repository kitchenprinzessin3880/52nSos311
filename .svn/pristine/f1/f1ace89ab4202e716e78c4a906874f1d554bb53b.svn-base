...
/**
 * class represents a UpdateSensorResponse
 * 
 * @author Stephan Kuenster
 * 
 */
public class UpdateSensorResponse implements ISosResponse {

    /** The RegisterSensorResponse XMLBeans document. */
    private UpdateSensorResponseDocument xb_upSensRespDoc;

    /** indicator for compression usage */
    private boolean applyZipCompression;

    /**
     * creates a new SosResponse for a updateSensor request.
     * 
     * @param respDoc
     *        the UpdateSensorResponseDocument.
     * @param applyZipCompression
     *        indicates if zip compression should be applied
     */
    public UpdateSensorResponse(UpdateSensorResponseDocument respDoc, boolean applyZipCompression) {
        this.xb_upSensRespDoc = respDoc;
        XmlCursor cursor = this.xb_upSensRespDoc.newCursor();
     ...
     *        the UpdateSensorResponseDocument.
     */
    public UpdateSensorResponse(UpdateSensorResponseDocument respDoc) {
        this.xb_upSensRespDoc = respDoc;
        XmlCursor cursor = this.xb_upSensRespDoc.newCursor();
        if (cursor.toFirstChild()) {
            cursor.setAttributeText(new QName("http://www.w3.org/2001/XMLSchema-instance",
    ...
    public byte[] getByteArray() throws IOException {
        ...
        xb_upSensRespDoc.save(baos, options);
