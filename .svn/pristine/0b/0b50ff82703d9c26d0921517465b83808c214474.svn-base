...
//            throw se;
    ...
    public AbstractSosRequest parseInsertObsRequest(InsertObservationDocument xb_insertObsDoc) throws OwsExceptionReport {
        ...
        if (assignedSensorID == null) {
            String message = "The mandatory parameter 'assignedSensorID' is missing in the InsertObservation request!!";
            OwsExceptionReport se = new OwsExceptionReport();
            se.addCodedException(ExceptionCode.InvalidParameterValue, null, message);
            log.error(message);
            throw se;
        }
        ...
        if (xb_obsType instanceof MeasurementType) {
            obsCol = omDecoder.parseMeasurement((MeasurementType) xb_obsType, false);
        }
        else if (xb_obsType instanceof CategoryObservationType) {
            obsCol = omDecoder.parseCategoryObservation((CategoryObservationType) xb_obsType, false);
        }
        else if (xb_obsType instanceof GeometryObservationType) {
            obsCol = omDecoder.parseSpatialObservation((GeometryObservationType) xb_obsType, false);
        }
        else if (xb_obsType instanceof ObservationType) { // Generic Observation
            obsCol = omDecoder.parseGenericObservation(xb_obsType, false);
        }
        return new SosInsertObservationRequest(assignedSensorID, obsCol, false);
    }
    ...
    public SosRegisterSensorRequest parseRegisterSensorDocument(RegisterSensorDocument xb_regSensDoc) throws OwsExceptionReport {
        ...
        try {
            ...
            // TODO add schema location here!
            XmlOptions options = new XmlOptions();
            options.setSaveNamespacesFirst();
            options.setSaveAggressiveNamespaces();
            smlFile = xb_sensorML.xmlText(options);
    ...
    public AbstractSosRequest parseGetFoiRequest(GetFeatureOfInterestDocument xb_getFoiDoc) throws OwsExceptionReport {
        ...
        if (xb_getFoi.getEventTimeArray() != null && xb_getFoi.getEventTimeArray().length != 0) {
            ...
            throw se;
            // getFoiRequest.setEventTimes(parseEventTime4GetFoi((net.opengis.sos.x10.GetFeatureOfInterestDocument.GetFeatureOfInterest.EventTime[])
            // xb_getFoi
            // .getEventTimeArray()));
        }
    ...
    public AbstractSosRequest parseGetObservationRequest(GetObservationDocument xb_obsDoc) throws OwsExceptionReport {
        ...
        /*
         * request.setRequestString(xb_obsDoc.toString());
         * 
         ...
         * (SosConstants.ResultType.contains(resultTypeString)) { request.setResultType
         * (SosConstants.ResultType.valueOf(resultTypeString)); } else { ServiceException se = new
         * ServiceException(); se.addCodedException(ServiceException .ExceptionCode.InvalidParameterValue,
    ...
    public AbstractSosRequest parseGetCapabilitiesRequest(GetCapabilitiesDocument xb_capsDoc) throws OwsExceptionReport {
        ...
        // TODO not implemented, see
        // org.n52.sos.ds.pgsql.PGSQLGetCapabilitiesDAO.getUpdateSequence(CapabilitiesDocument)
    ...
    /**
     * parses the eventTime of the GetObservation requests and returns an array representing the temporal
     * filters
     * 
     * @param xb_eventTimes
    ...
    private SpatialFilter parseLocation(Location xb_location) throws OwsExceptionReport {
        ...
        // if not ogc:BBOX
            ...
            // switch between node names because type is now BinarySpatialOpType
    ...
    private SpatialFilter parseFeatureOfInterest(FeatureOfInterest xb_foi) throws OwsExceptionReport {
        ...
        if (xb_foi == null) {
            ...
            // if not ogc:BBOX
                ...
                // switch between node names because type is now
                // BinarySpatialOpType - no use of instanceof
                // operator possible :-(
