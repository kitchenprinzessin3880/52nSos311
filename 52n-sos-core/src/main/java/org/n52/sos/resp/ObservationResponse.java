...
/**
 * Implementation of the ISosResponse interface for a response to a getObservation request.
 * 
...
public class ObservationResponse implements ISosResponse {
     ...
     */
    public ObservationResponse(ObservationCollectionDocument obsDoc, boolean applyZipCompression) {
        this.obsColDoc = obsDoc;
        XmlCursor cursor = this.obsColDoc.newCursor();
        if (cursor.toFirstChild()) {

            String schemaLocation = OMConstants.NS_OM + " " + OMConstants.SCHEMA_LOCATION_OM + " "
                    + OMConstants.NS_SA + " " + OMConstants.SCHEMA_LOCATION_SAMPLING;
     ...
     *        indicates if zip compression should be applied
     */
    public ObservationResponse(ByteArrayOutputStream docOutputStream, boolean applyZipCompression) {
    ...
    public byte[] getByteArray() throws IOException {
        ...
        if (obsColDoc != null) {
            ...
            /*
             * workaround for xmlns:xlink prefix, cause in any schema the name of the xlink prefix is wrong,
             * so set this on xlink!
             */
            ...
            this.obsColDoc.save(baos, options);
