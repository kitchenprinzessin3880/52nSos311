...
public class SosConstants {
    ...
    /** enum with parameter names for getObservation request */
    public enum GetObservationParams { //modified by ASD
        qualityFlag,service, version, srsName, resultType, startPosition, maxRecords, offering, eventTime, procedure, observedProperty, domainFeature, featureOfInterest, result, responseFormat, resultModel, responseMode, SortBy, BBOX;
        ...
        public static boolean contains(String s) {
            ...
            contained = (s.equals(GetObservationParams.service.name()))
                    ...
                    || (s.equals(GetObservationParams.responseMode.name()) 
                    		//modified by ASD
                    || (s.equals(GetObservationParams.qualityFlag.name())));
            return contained;
        }
    }
